...
void AdminWindow::initAdminWindow(){
	...
	redo_shortcut = new QShortcut(QKeySequence("Ctrl+Y"), this);
}
...
void AdminWindow::ConnectSignals() {
	...
	connect(undo_button, &QPushButton::clicked, this, &AdminWindow::UndoButtonHandler);
	connect(redo_button, &QPushButton::clicked, this, &AdminWindow::RedoButtonHandler);
	connect(undo_shortcut, &QShortcut::activated, this, &AdminWindow::UndoButtonHandler);
	connect(redo_shortcut, &QShortcut::activated, this, &AdminWindow::RedoButtonHandler);
}
...
void AdminWindow::GenerateChartButtonHandler(){
	...
	*set << service.GetNumberPetAge(1, 3)
	...
	chart_view->setWindowIcon(QIcon("chart.png"));
	chart_view->show();
}
...
void AdminWindow::UndoButtonHandler(){
	if (!service.NotUndo()) {
		...
		QMessageBox::critical(this, "Error", "Not more undos");
}
...
void AdminWindow::RedoButtonHandler(){
	if (!service.NotRedo()) {
		...
		QMessageBox::critical(this, "Error", "Not more redos");
}
