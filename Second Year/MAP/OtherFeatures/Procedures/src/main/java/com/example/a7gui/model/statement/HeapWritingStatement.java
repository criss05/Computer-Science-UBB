...
public class HeapWritingStatement implements IStatement {
    ...
    public PrgState execute(PrgState prgState) throws StatementException, ADTException, ExpressionException, VariableAlreadyExists {
        if (!prgState.getCurrentSymTable().contains(String.valueOf(this.var_name))) {
            throw new StatementException("The variable does not exists in the heap!");
        }

        IValue var_val=prgState.getCurrentSymTable().getValue(this.var_name);
        ...
        IValue eval_value = this.expression.eval(prgState.getCurrentSymTable(), prgState.getHeap());
