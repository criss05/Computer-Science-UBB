...
import com.example.a7gui.model.expression.ArithmeticalExpression;
import com.example.a7gui.model.expression.ArithmeticalOperator;
...
public class ProgramPageController {
    ...
    public void initialize(){
        Platform.runLater(()->{
            ...
            try {
                this.currentPrgState.getProcTable().insert("sum", new Pair<>(new ArrayList<>(Arrays.asList("a", "b")), new CompStatement(new VarDeclStatement("v", new IntType()),new CompStatement(new AssignmentStatement("v", new ArithmeticalExpression(new VariableExpression("a"), ArithmeticalOperator.ADD, new VariableExpression("b"))), new PrintStatement(new VariableExpression("v"))))));
                this.currentPrgState.getProcTable().insert("product", new Pair<>(new ArrayList<>(Arrays.asList("a", "b")), new CompStatement(new VarDeclStatement("v", new IntType()),new CompStatement(new AssignmentStatement("v", new ArithmeticalExpression(new VariableExpression("a"), ArithmeticalOperator.MULTIPLY, new VariableExpression("b"))), new PrintStatement(new VariableExpression("v"))))));
    ...
    private void populateUncommonViews() {
        this.symTableView.getItems().setAll(this.currentPrgState.getCurrentSymTable().getMap().entrySet().stream().map(e-> new SymTableViewModel(e.getKey(), e.getValue())).toList());
