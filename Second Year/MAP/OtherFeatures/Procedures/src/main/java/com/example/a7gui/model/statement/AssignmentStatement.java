...
public class AssignmentStatement implements IStatement{
    ...
    public PrgState execute(PrgState prgState) throws StatementException, ADTException, ExpressionException {
        if(!prgState.getCurrentSymTable().contains((this.variableName)))
            throw new ADTException("Variable was not found!");

        IValue value = prgState.getCurrentSymTable().getValue(this.variableName);
        IValue evalValue = this.expression.eval(prgState.getCurrentSymTable(), prgState.getHeap());
        if (!value.getType().equals(evalValue.getType())){
            throw new StatementException("Value type is not ok.");
        }
        prgState.getCurrentSymTable().insert(this.variableName, evalValue);
        return null;
    }
