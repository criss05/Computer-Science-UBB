...
class Graph:
    ...
    def edge_cover_aprox(self):
        """
        Finds an approximate vertex cover of the graph using a 2-approximation algorithm
        ...
        while edges:
            # Pick any edge (u, v)
            (vertex1, vertex2) = edges.pop()

            # Add u and v to the vertex cover
            vertex_cover.add(vertex1)
            vertex_cover.add(vertex2)

            # Remove all edges incident on either u or v
            edges_to_remove = set()
            for edge in edges:
                if vertex1 in edge or vertex2 in edge:
